...
class GroqProvider:
    ...
    def __init__(self, api_key: str | List[str], model: str = "meta-llama/llama-4-scout-17b-16e-instruct"):
        """
    ...
    def rotate_key(self):
        ...
        if len(self._api_keys) > 1:
            ...
            logger.info(f"Rotating Groq API key. New index: {self._key_index}")
            # Log which key is being used now (masked for safety)
            current_key = self.api_key
            masked_key = f"{current_key[:6]}...{current_key[-4:]}" if len(current_key) > 10 else "***"
            logger.info(f"Now using Groq key: {masked_key}")
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.warning(
            f"Retry Groq request ({retry_state.attempt_number}/5) - "
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 429:
                logger.warning(f"Groq API Quota Exhausted (429) for key index {self._key_index}")
                self.rotate_key()
            ...
            if response.status_code >= 400:
                ...
                if response.status_code == 400:
                    logger.error("Check if the model supports vision or if the prompt is valid.")
                response.raise_for_status()
